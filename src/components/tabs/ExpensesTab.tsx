...
const fmtKRW = (n: number) =>
  n >= 100000000 ? `${(n / 100000000).toFixed(2)}억` : `${n.toLocaleString()}원`;
...
export default function ExpensesTab() {
  ...
  const [form, setForm] = useState({ category: CATEGORIES[0], description: "", amount: "", date: new Date().toISOString().slice(0, 10) });
  ...
  const addExpense = async () => {
    ...
    const res = await fetch("/api/expenses", {
      ...
      body: JSON.stringify({ category: form.category, description: form.description, amount: Number(form.amount.replace(/,/g, "")), date: form.date }),
    });
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="flex flex-wrap gap-2">
          {availableMonths.slice(0, 6).map((m) => (
            <button key={m} onClick={() => setFilterMonth(m)}
              className={`px-3 py-1.5 rounded-full text-sm font-medium transition-colors ${filterMonth === m ? "bg-black text-white" : "bg-white border border-gray-200 text-gray-600 hover:border-gray-400"}`}>
              {m.slice(5)}월
            </button>
          ))}
        </div>
          ...
          <CardContent className="pt-4">
            <div className="flex flex-wrap gap-2 items-end">
              <div>
                ...
                <select className="border border-gray-200 rounded px-2 py-1.5 text-sm" value={form.category}
              ...
              <div>
                ...
                <Input className="w-32" placeholder="예: 500000" value={form.amount}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        {catSummary.map((c) => (
          <Card key={c.cat}>
            <CardHeader className="pb-1"><CardTitle className="text-xs text-gray-500">{c.cat}</CardTitle></CardHeader>
            <CardContent>
      ...
      <div className="flex flex-wrap gap-2">
        {["전체", ...CATEGORIES].map((c) => (
          <button key={c} onClick={() => setFilterCat(c)}
            className={`px-3 py-1 rounded-full text-xs font-medium transition-colors ${filterCat === c ? "bg-black text-white" : "bg-white border border-gray-200 text-gray-600 hover:border-gray-400"}`}>
            {c}
          </button>
        ))}
      </div>
      ...
      <Card>
        <CardContent className="pt-4">
          {filtered.length === 0 && <p className="text-sm text-gray-400 text-center py-8">{filterMonth.slice(5)}월에 등록된 비용이 없어요</p>}
          <div className="space-y-2">
            {filtered.sort((a, b) => b.date.localeCompare(a.date)).map((e) => (
              <div key={e.id} className="flex items-center gap-3 p-3 rounded-lg border border-gray-100 hover:border-gray-200 bg-white">
                <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${CATEGORY_COLORS[e.category] || "bg-gray-100 text-gray-700"}`}>{e.category}</span>
                ...
                <p className="text-xs text-gray-400">{new Date(e.date).toLocaleDateString("ko-KR", { month: "numeric", day: "numeric" })}</p>
